...
def model_opts(parser):
    ...
    group.add('--encoder_type', '-encoder_type', type=str, default='rnn',
              choices=['rnn', 'brnn', 'mean', 'transformer', 'cnn', 
                       'transformer_boost', 'transformer_multi_task', 'transformer_dense',
                       'mlm_transformer', 'transformer_boost_new'],
              help="Type of encoder layer to use. Non-RNN layers "
                   "are experimental. Options are "
                   "[rnn|brnn|mean|transformer|cnn].")
    group.add('--decoder_type', '-decoder_type', type=str, default='rnn',
              choices=['rnn', 'transformer', 'cnn', 'transformer_multi_task', 'transformer_causal'],
    ...
    # Boost Model's options.
    group.add('--num_boost', '-num_boost', type=int, default=4,
              help='Number of boosts for transformer self-attention')
    group.add('--boost_type', '-boost_type',
              type=str, default="continuous",
              help='Boost type in self attention nets. ')
    group.add('--boost_main', '-boost_main',
              action="store_true",
              help='boost contains the main stream.')
    group.add('--boost_drop_rate', '-boost_drop_rate', type=float, default=1.0,
              help="drop rate for boost models.")
    group.add('--boost_dropout_diff', '-boost_dropout_diff', type=float, default=0.0,
              help="drop diff for each boost modules.")
    group.add('--self_att_orthogonal', '-self_att_orthogonal',
              action="store_true",
              help='orthogonal init in self-attention layer.')
    group.add('--boost_gating', '-boost_gating',
              action="store_true",
              help='enable boost gating.')
    group.add('--boost_with_ffn', '-boost_with_ffn',
              action="store_true",
              help='whether also to boost with ffn.')
    group.add('--use_postag', '-use_postag',
              action="store_true",
              help='whether also to boost with postag.')
    group.add('--disable_learnable_weights', '-disable_learnable_weights',
              action="store_true",
              help='whether to disable the learnable weights of different path.')
    group.add('--self_att_merge_layer', '-self_att_merge_layer',
              action="store_true",
              help='whether to use self_att_merge_layer.')
    group.add('--boost_str', '-boost_str', default='', type=str,
              help="string to indicate boost types.")
    group.add('--mask_pos_type', '-mask_pos_type', default=[], nargs='*',
              help="list for masked postag type.")
    group.add('--shuffle_merge', '-shuffle_merge',
              action="store_true",
              help='whether to enable shuffle merge.')
    group.add('--shuffle_merge_type', '-shuffle_merge_type', default='sum', type=str,
              help="ways for shuffle merge.")
    group.add('--extra_loss_weight', '-extra_loss_weight', type=float, default=0.0,
              help="extra loss weight. if > 0, enable ExtraLossGenerator")
    group.add('--boost_adv_method_list', '-boost_adv_method_list', default=[], nargs='*',
              help="list for boost adversarial methods.")
    group.add('--boost_sample_rate', '-boost_sample_rate', type=float, default=1.0,
              help="sample rate between distorted input and golden input")
    group.add('--shuffle_fix', '-shuffle_fix', type=int, default=1,
              help="shuffle fix step for training shuffle merge.")
    group.add('--boost_single_att', '-boost_single_att',
              action="store_true",
              help='boost contains the main stream.')
    group.add('--boost_single_ffn', '-boost_single_ffn',
              action="store_true",
              help='boost contains the main stream.')
    group.add('--shuffle_stop_gradient', '-shuffle_stop_gradient',
              action="store_true",
              help='boost contains the main stream.')
#     group.add('--boost_single_ffn', '-boost_single_ffn', type=bool, default=False,
          #     help="whether to use only one feed forward.")
    group.add('--eval_only', '-eval_only', type=bool, default=False,
              help="only to validate the model.")

    group.add('--enable_adversarial_training', '-enable_adversarial_training',
              action="store_true",
              help='whether to enable adversarial training.')
    group.add('--adv_gradient_boost', '-adv_gradient_boost',
              action="store_true",
              help='whether to enable adversarial gradient boost.')
    group.add('--adv_gradient_boost_step', '-adv_gradient_boost_step', type=float, default=0.01,
              help="adv gradient step for different paths.")
    # adv_gradient_boost_func
    group.add('--adv_gradient_boost_func', '-adv_gradient_boost_func', type=str, default="mse",
              help="adv gradient boost loss function for different paths.")

    group.add('--adv_bias_step', '-adv_bias_step', type=float, default=0.0,
              help="adv bias step for different paths.")
    group.add('--adv_bias_layer', '-adv_bias_layer', type=int, default=0,
              help="adv bias step layer enabled for boost model.")
    group.add('--adv_gradient_boost_no_ce', '-adv_gradient_boost_no_ce',
              action="store_true",
              help='whether to stop cross entropy for adv_gradient path.')
    # adv_gradient_boost_scale
    group.add('--adv_gradient_boost_scale', '-adv_gradient_boost_scale', type=float, default=0.01,
              help="adv bias loss scale for different paths.")
    ...
    group.add('--multi_task', '-multi_task', action="store_true",
              help="Multi-task framework for loss computation.")
    group.add('--loss_every_layers', '-loss_every_layers', type=int, default=2,
              help='Every n layer to computing the loss.')
    group.add('--cross_module', '-cross_module', action="store_true",
              help="Whether to use cross module links in multi-task decoder.")
    group.add('--multi_anneal', '-multi_anneal', action="store_true",
              help="Whether to anneal each multi-task loss in loss computation.")

    # for dense model.
    group.add('--dense_growth_rate', '-dense_growth_rate', type=int, default=64,
              help='Number of growth rate in dense nets.')
    group.add('--dense_block_dim', '-dense_block_dim', type=int, default=64,
              help='Base dim in dense nets.')
    group.add('--num_dense_layers', '-num_dense_layers', type=int, default=6,
...
def preprocess_opts(parser):
    ...
    group.add('--train_src_pos', '-train_src_pos', nargs='+', default=[None],
              help="Path(s) to the training source pos-tag data")
    ...
    group.add('--valid_src_pos', '-valid_src_pos', default=[None],
              help="Path(s) to the valid source pos-tag data")
    ...
    group.add('--tgt_vocab', '-tgt_vocab', default="",
              help="Path to an existing target vocabulary. Format: "
                   "one word per line.")
    group.add('--src_pos_vocab', '-src_pos_vocab', default="",
              help="Path to an existing source postag vocabulary. Format: "
                   "one word per line.")
    ...
    group.add('--src_pos_vocab_size', '-src_pos_vocab_size', type=int, default=1000,
              help="Size of the source pos vocabulary")
    ...
    group.add('--src_pos_words_min_frequency',
              '-src_pos_words_min_frequency', type=int, default=0)
...
def translate_opts(parser):
    ...
    group.add('--src', '-src', required=True,
              help="Source sequence to decode (one line per "
                   "sequence)")
    group.add('--src_pos', '-src_pos', default="",
              help="Source pos sequence to decode (one line per "
                   "sequence)")
