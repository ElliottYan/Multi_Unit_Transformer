...
def get_fields_with_postag(
    ...
    fields = get_fields(src_data_type, n_src_feats, n_tgt_feats, 
                            pad=pad, bos=bos, eos=eos, dynamic_dict=dynamic_dict,
                            with_align=with_align, src_truncate=src_truncate,
                            tgt_truncate=tgt_truncate)

    src_pos_field_kwargs = {"n_feats": n_src_pos_feats,
...
def _build_fields_vocab_with_postag(fields, counters, data_type, share_vocab,
                                    vocab_size_multiple,
                                    src_vocab_size, src_words_min_frequency,
                                    src_pos_vocab_size, src_pos_words_min_frequency,
                                    tgt_vocab_size, tgt_words_min_frequency):
    build_fv_args = defaultdict(dict)
    build_fv_args["src"] = dict(
        max_size=src_vocab_size, min_freq=src_words_min_frequency)
    build_fv_args["src_pos"] = dict(
        max_size=src_pos_vocab_size, min_freq=src_pos_words_min_frequency)
    ...
    if data_type == 'text':
        ...
        _build_fv_from_multifield(
            ...
            build_fv_args,
            size_multiple=vocab_size_multiple if not share_vocab else 1)
        src_pos_multifield = fields["src_pos"]
        _build_fv_from_multifield(
            src_pos_multifield,
            counters,
            build_fv_args,
