...
from onmt.inputters.inputter import _build_fields_vocab, _build_fields_vocab_with_postag, \
                                    _load_vocab
...
def process_one_shard(corpus_params, params):
    corpus_type, fields, src_reader, src_pos_reader, tgt_reader, align_reader, opt,\
         existing_fields, src_vocab, src_pos_vocab, tgt_vocab = corpus_params
    i, (src_shard, src_pos_shard, tgt_shard, align_shard, maybe_id, filter_pred) = params
    ...
    src_pos_data = {"reader": src_pos_reader, "data": src_pos_shard, "dir": None}
    tgt_data = {"reader": tgt_reader, "data": tgt_shard, "dir": None}
    align_data = {"reader": align_reader, "data": align_shard, "dir": None}
    _readers, _data, _dir = inputters.Dataset.config(
        [('src', src_data), ('src_pos', src_pos_data), ('tgt', tgt_data), ('align', align_data)])
...
def maybe_load_vocab(corpus_type, counters, opt):
    ...
    if corpus_type == "train":
        ...
        if opt.src_pos_vocab != "":
            src_pos_vocab, src_pos_vocab_size = _load_vocab(
                opt.src_pos_vocab, "src_pos", counters,
                opt.src_pos_words_min_frequency)
    return src_vocab, src_pos_vocab, tgt_vocab, existing_fields


def build_save_dataset(corpus_type, fields, src_reader, src_pos_reader, tgt_reader,
                       align_reader, opt):
    ...
    if corpus_type == 'train':
        ...
        counters = None
        srcs = [opt.valid_src]
        src_poses = [opt.valid_src_pos]
        tgts = [opt.valid_tgt]
        ids = [None]
        aligns = [opt.valid_align]

    # modified
    src_vocab, src_pos_vocab, tgt_vocab, existing_fields = maybe_load_vocab(
    ...
    def shard_iterator(srcs, src_poses, tgts, ids, aligns, existing_shards,
                       existing_fields, corpus_type, opt):
        """
        Builds a single iterator yielding every shard of every corpus.
        """
        for src, src_pos, tgt, maybe_id, maybe_align in zip(srcs, src_poses, tgts, ids, aligns):
            ...
            for i, (ss, sp, ts, a_s) in enumerate(
                    zip(src_shards, src_pos_shards, tgt_shards, align_shards)):
                yield (i, (ss, sp, ts, a_s, maybe_id, filter_pred))

    shard_iter = shard_iterator(srcs, src_poses, tgts, ids, aligns, existing_shards,
                                existing_fields, corpus_type, opt)

    with Pool(opt.num_threads) as p:
        dataset_params = (corpus_type, fields, src_reader, src_pos_reader, tgt_reader,
                          align_reader, opt, existing_fields,
                          src_vocab, src_pos_vocab, tgt_vocab)
    ...
    if corpus_type == "train":
        ...
        if existing_fields is None:
            fields = _build_fields_vocab_with_postag(
                fields, counters, opt.data_type,
                opt.share_vocab, opt.vocab_size_multiple,
                opt.src_vocab_size, opt.src_words_min_frequency,
                opt.src_pos_vocab_size, opt.src_pos_words_min_frequency,
...
def preprocess(opt):
    ...
    tgt_nfeats = 0
    src_pos_nfeats = 0
    for src, src_pos, tgt in zip(opt.train_src, opt.train_src_pos, opt.train_tgt):
        src_nfeats += count_features(src) if opt.data_type == 'text' \
            else 0
        src_pos_nfeats += count_features(src_pos) if opt.data_type == 'text' \
            else 0
        tgt_nfeats += count_features(tgt)  # tgt always text so far
    logger.info(" * number of source features: %d." % src_nfeats)
    logger.info(" * number of source postag features: %d." % src_pos_nfeats)
    logger.info(" * number of target features: %d." % tgt_nfeats)

    logger.info("Building `Fields` object...")
    fields = inputters.get_fields_with_postag(
        opt.data_type,
        src_nfeats,
        src_pos_nfeats,
        tgt_nfeats,
    ...
    build_save_dataset(
        'train', fields, src_reader, src_pos_reader, tgt_reader, align_reader, opt)

    if opt.valid_src and opt.valid_tgt:
        logger.info("Building & saving validation data...")
        build_save_dataset(
            'valid', fields, src_reader, src_pos_reader, tgt_reader, align_reader, opt)
