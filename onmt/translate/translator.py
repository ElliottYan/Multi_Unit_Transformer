...
class Translator(object):
    ...
    def translate(
        ...
        report_model_dist = False
        if report_model_dist:
            if hasattr(self.model.encoder.transformer[0], 'keep_ffn_dist') and self.model.encoder.transformer[0].keep_ffn_dist:
                # print out mean of keep_ffn_dist
                for i in range(len(self.model.encoder.transformer)):
                    keep_ffn_dist = self.model.encoder.transformer[i].keep_ffn_dist
                    print('Mean ffn dist in encoder layer {}: {} '.format(i, sum(keep_ffn_dist)/len(keep_ffn_dist)))
            if hasattr(self.model.encoder.transformer[0], 'keep_attn_dist') and self.model.encoder.transformer[0].keep_attn_dist:
                # print out mean of keep_ffn_dist
                for i in range(len(self.model.encoder.transformer)):
                    keep_attn_dist = self.model.encoder.transformer[i].keep_attn_dist
                    print('Mean attn dist in encoder layer {}: {} '.format(i, sum(keep_attn_dist)/len(keep_attn_dist)))
            if hasattr(self.model.encoder.transformer[0], 'keep_attn_out_dist') and self.model.encoder.transformer[0].keep_attn_out_dist:
                # print out mean of keep_ffn_dist
                for i in range(len(self.model.encoder.transformer)):
                    keep_attn_out_dist = self.model.encoder.transformer[i].keep_attn_out_dist
                    print('Mean attn out dist in encoder layer {}: {} '.format(i, sum(keep_attn_out_dist)/len(keep_attn_out_dist)))
