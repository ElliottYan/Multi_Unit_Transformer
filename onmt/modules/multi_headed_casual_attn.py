...
class MultiHeadedCausalAttention(nn.Module):
    ...
    def __init__(self, head_count, model_dim, dropout=0.1,
                 max_relative_positions=0, orthogonal=False):
        assert model_dim % head_count == 0
        self.dim_per_head = model_dim // head_count
        self.model_dim = model_dim

        super(MultiHeadedCausalAttention, self).__init__()
        ...
        if orthogonal:
            torch.nn.init.orthogonal_(self.linear_keys.weight)
            torch.nn.init.orthogonal_(self.linear_values.weight)
            torch.nn.init.orthogonal_(self.linear_query.weight)
            torch.nn.init.orthogonal_(self.final_linear.weight)

    def forward(self, key, value, query, mask=None,
                layer_cache=None, attn_type=None, decoder=False):
        """
        ...
        if use_causal:
            ...
            bk_mask = 1.0 - torch.diag(torch.ones(query_len, device=mask.device)).unsqueeze(0).repeat([batch_size, 1, 1]).to(mask.dtype)
            # [bz, len, len]
            bk_mask = (bk_mask + mask).gt(0).to(query_key.dtype)
            # [bz, 1, len, len]
            incre_mask = (bk_mask.to(mask.dtype) - mask).to(query_key.dtype).unsqueeze(1)
            # [bz, 1, len, len]
        ...
        if use_causal:
            ...
            context_original = bk_mask.unsqueeze(-1) * context_original + incre_mask.unsqueeze(-1) * context_original_detach
