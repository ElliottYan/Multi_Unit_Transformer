...
def build_loss_compute(model, tgt_field, opt, train=True):
    ...
    # elif opt.extra_loss_weight > 0.0:
        # only support NLLLoss now in the extra loss computation.
    ...
    if opt.copy_attn:
        ...
        compute = MultiTaskNMTLossCompute(
            criterion, loss_gen, lambda_coverage=opt.lambda_coverage,
            lambda_align=opt.lambda_align, multi_task=opt.multi_task, 
            loss_every_layers=opt.loss_every_layers,
            dec_layers=opt.dec_layers, train_steps=opt.train_steps,
            anneal=opt.multi_anneal)
    elif opt.extra_loss_weight > 0.0:
        compute = onmt.modules.NMTLossComputeWithExtraLoss(
            criterion, loss_gen, lambda_coverage=opt.lambda_coverage,
            lambda_align=opt.lambda_align, multi_task=opt.multi_task,
            extra_loss_weight=opt.extra_loss_weight)
    else:
...
class LossComputeBase(nn.Module):
    ...
    def __call__(self,
        ...
        for shard in shards(shard_state, shard_size):
            if self.__class__ == MultiTaskNMTLossCompute:
...
class MultiTaskNMTLossCompute(NMTLossCompute):
    """
    Multi-task NMT Loss Computation.
    """

    def __init__(self, criterion, generator, normalization="sents",
                 lambda_coverage=0.0, lambda_align=0.0, multi_task=False, loss_every_layers=2, dec_layers=6,
                 train_steps=10000, anneal=True):
        super(MultiTaskNMTLossCompute, self).__init__(criterion, generator, lambda_coverage, lambda_align)
        self.multi_task = multi_task
        self.loss_every_layers = loss_every_layers
        self.dec_layers = dec_layers
        self.train_steps = train_steps
        self.anneal = anneal

    def _compute_loss(self, batch, target, std_attn=None,
                      coverage_attn=None, align_head=None, ref_align=None, **kwargs):
        outputs = dict()
        for key in kwargs:
            if key.startswith('output_'):
                outputs[key] = kwargs[key]

        _total = len(outputs)
        _loss_on = list(range(self.loss_every_layers-1, _total, self.loss_every_layers))
        losses = []
        stats = []
        for i in _loss_on:
            each_loss, each_stat = self._compute_loss_single(batch, outputs['output_{}'.format(i)], target, std_attn=std_attn,
                                                             coverage_attn=coverage_attn, align_head=align_head, ref_align=ref_align)
            # compute the decay rate for each intermediate layer.
            if kwargs.get('step') and i != self.dec_layers - 1:
                step = kwargs.get('step')
                total_step = self.train_steps
                if self.anneal:
                    decay_rate = 1.0 - (2.0 * self.dec_layers * (step+1) / (total_step * (i+1)))
                    decay_rate = max(0, decay_rate)
                else:
                    decay_rate = 1.0 if step <= int(total_step * (i+1) / (2.0 * self.dec_layers)) else 0.0
            # last layer loss does not decay.
            else:
                decay_rate = 1.0
            each_loss = each_loss * decay_rate
            losses.append(each_loss)
            stats.append(each_stat)
        default = True
        if default:
            loss = torch.stack(losses).sum()
            num_non_padding = stats[0].n_words
            num_correct = stats[-1].n_correct
            final_stat = onmt.utils.Statistics(loss.item(), num_non_padding, num_correct)
        return loss, final_stat

    def _compute_loss_single(self, batch, output, target, std_attn=None,
                             coverage_attn=None, align_head=None, ref_align=None):
    ...
    def _make_shard_state(self, batch, output, range_, attns=None):
        ...
        if self.multi_task is True:
            for i in range(len(shard_state['output'])):
                shard_state['output_{}'.format(i)] = shard_state['output'][i]
            shard_state.pop('output')
        return shard_state
