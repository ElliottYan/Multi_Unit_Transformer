...
class TransformerBoostDecoderLayer(nn.Module):
    def __init__(self, d_model, heads, d_ff, dropout, attention_dropout,
                self_attn_type='scaled-dot', max_relative_positions=0, 
                aan_useffn=False, full_context_alignment=False,
                alignment_heads=None,
                num_boost=4, learnable_weights=True, 
                ...
                adv_bias_step=0.0, shuffle_merge=False):    
        super(TransformerBoostDecoderLayer, self).__init__()
        ...
        self.paths = [ 
            TransformerDecoderLayer(d_model, heads, d_ff, dropout_list[i],
        ...
        if self.use_adv or self.use_postag is True:
            ...
            self.activate_methods = set(all_adv_methods[:self.a_num])
            # create mask tensor
    ...
    def forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask, step=None, 
                dec_layer_grads=None, layer_cache=None, future=False, **kwargs):
        """Forward function for Boost Decoder Layer.
        
        Arguments:
            inputs {Tensor, [bz, tgt_len, dim]} -- Decoder input.
            memory_bank {Tensor, [bz, src_len, dim]} -- Encoder output.
            src_pad_mask {Tensor, [bz, src_len]} -- Mask for encoder output.
            tgt_pad_mask {Tensor, [bz, tgt_len]} -- Mask for decoder input.
        
        Keyword Arguments:
            step {[type]} -- [description] (default: {None})
            dec_layer_grads {[type]} -- [description] (default: {None})
            layer_cache {[type]} -- [description] (default: {None})
            future {bool} -- [description] (default: {False})
        
        Returns:
            [type] -- [description]
        """
        # NOTE: here we perform layer-norm before the inputs.
        input_norm = self.layer_norm(inputs)
        if self.training:
            input_norm_list, mask_list = self.create_input_norms(input_norm, tgt_pad_mask)
        else:
            input_norm_list = [input_norm for i in range(self.num_boost)]
            mask_list = [torch.ones_like(tgt_pad_mask).transpose(1,2) for i in range(self.num_boost)]

        mask_list = [(1.0 - item.transpose(1, 2).long()) for item in mask_list]
        mask_list = [(tgt_pad_mask.long()+mask_list[i]).gt(0.0).to(tgt_pad_mask.dtype) for i in range(len(mask_list))]
        outs = [self.layers[i](input_norm_list[i],
                memory_bank,
                src_pad_mask,
                tgt_pad_mask,
                layer_cache=layer_cache,
                step=step,
                with_align=with_align) for i in range(self.num_boost)]
        # bz, length, D, num_boost
        outs = torch.stack(outs, dim=-1)
        # call merge layer
        out, outs, extra_loss = self.merge_layer(input_norm, outs, enc_layer_grads, mask)
        ...
        # first pass
        if self.adv_gradient_boost and enc_layer_grads is None:
            ret['outs'] = outs
        return ret
...
class TransformerDecoderLayer(nn.Module):
    ...
    def _forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask,
        ...
        # NOTE: To accomondate usage in transformer encoder, we move the input_norm out.
