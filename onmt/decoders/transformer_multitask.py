...
class TransformerDecoderMultiTask(DecoderBase):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff,
                 ...
                 alignment_heads=None, cross_module=False):
        super(TransformerDecoderMultiTask, self).__init__()
        ...
        intermedia_layer_norm = [nn.LayerNorm(d_model, eps=1e-6) for i in range(num_layers-1)]
        self.intermedia_layer_norm = nn.ModuleList(intermedia_layer_norm)
    ...
    def forward(self, tgt, memory_bank, step=None, **kwargs):
        ...
        src_memory_bank = [memory.transpose(0, 1).contiguous() for memory in memory_bank]
        ...
        assert len(self.transformer_layers) == len(src_memory_bank)
        ret = []

        for i, layer in enumerate(self.transformer_layers):
            layer_cache = self.state["cache"]["layer_{}".format(i)] \
                if step is not None else None
            if self.cross_module is True:
                current_memory_bank = src_memory_bank[i]
            else:
                current_memory_bank = src_memory_bank[-1]
            ...
            ret.append(self.intermedia_layer_norm[i-1](output))

        output = self.layer_norm(output)
        # dec_outs = output.transpose(0, 1).contiguous()
        ret = ret[:-1]
        ret.append(output)
        ret = [item.transpose(0, 1).contiguous() for item in ret]
